...
\multicolumn{4}{C{13.1cm}}{Continued on next page}\\
\endfoot
...
FR-16-TC-02&UST&Test if an end user is able to login to a provided user (the user has been created) within 2 minutes&Passed \cellcolor{green!60} \\ \hline
...
FR-18-TC-02&UST&Test that a user who is already logged in can edit and update his own Account Information within 2 minutes&Failed\footnote{The User thought the Account Information was saved but it wasn't because a required field was omitted} \cellcolor{red!60} \\ \hline
...
FR-22-TC-02&UST&Test that a user who is already logged in can log out within 2 minutes&Passed \cellcolor{green!60} \\ \hline
...
FR-24-38-TC-01&AT&Ensure that Media Items must have the attributes described in FR-24-30 and that they can have the attributes described in FR-31 to FR-38&Failed\footnoteref{note1} \cellcolor{red!65} \\ \hline
FR-39-TC-01&AT&Test that a logged in user can upload a media item&Passed \cellcolor{green!65} \\ \hline
FR-39-TC-02&UST&Test that a user who is already logged in can upload a media item within 3 minutes&Passed \cellcolor{green!60} \\ \hline
...
FR-41-TC-01&AT&Test that if upload of a Media Item fails then no Media Item Information is saved&Passed \cellcolor{green!65} \\ \hline
...
FR-44-TC-04&UST&Test that a user who is already logged in can purchase a media item within 2 minutes if presented with a details page&Passed \cellcolor{green!60} \\ \hline
...
FR-45-TC-02&UST&Test that a user who is already logged in and has purchased a given Media Item, can start downloading this item within 2 minutes if presented with a details page&Passed \cellcolor{green!60} \\ \hline
...
FR-48-TC-03&UST&Test that a user who is already logged in can rate a Media Item which he has purchased within 2 minutes if presented with a details page&Failed\footnote{The User did in fact rate the \textit{Media Item} but he never realized that he had completed the task} \cellcolor{red!60} \\ \hline
...
FR-57-TC-01&AT&Given a Media Item Type, test that a list of promoted items, of the correct Media Item Type, is returned&Failed\footnote{Out of scope} \cellcolor{red!65} \\ \hline
...
FR-63-TC-01&UNT&Given a requesting user and a user id, test if a UserDTO with all information about the user with the given id is returned (except the password) when the id is different from the requesting user's id and the requesting user is not admin.&Passed \cellcolor{green!65} \\ \hline
...
FR-69-TC-01&UNT&Test if an exception is thrown if the username is not unique (case insensitive matching).&Passed \cellcolor{green!65} \\ \hline
FR-70-TC-01&UNT&Test if an exception is thrown if the username does not consist of between 1 and 20 characters (both included).&Passed \cellcolor{green!65} \\ \hline
FR-71-TC-01&UNT&Test if an exception is thrown if the username does not only contains alphanumeric characters (a-zA-Z0-9).&Passed \cellcolor{green!65} \\ \hline
FR-72-TC-01&UNT&Test if an exception is thrown if the password does not consist of between 1 and 20 characters (both included).&Passed \cellcolor{green!65} \\ \hline
FR-73-TC-01&UNT&Test if an exception is thrown if the password contains any whitespace character.&Passed \cellcolor{green!65} \\ \hline
...
FR-75-TC-01 & UNT & Given a valid \textit{Media Item} ID and a stream containing a thumbnail image, test that a reference to the thumbnail image is added to the \textit{Media Item}s \textit{Media Item Information} & Passed \cellcolor{green!65} \\ \hline
FR-75-TC-02 & IT & Given a valid \textit{Media Item} ID and a stream containing a thumbnail image, test that the thumbnail image is saved to a file & Passed \cellcolor{green!65} \\ \hline
...
FR-85-TC-01&UNT&Given a User and a request the User has rights to perform, test that the User is given access &Failed\footnote{Went with different solution \label{note3}} \cellcolor{red!65} \\ \hline
FR-85-TC-02&UNT&Given a User and a request the User does not have the rights to perform, test that the User is denied access&Failed\footnoteref{note3} \cellcolor{red!65} \\ \hline
FR-86-TC-01&UNT&Given a User and a request the User does not have the rights to perform, test that a response describing the problem is sent to the Client&Failed\footnoteref{note3} \cellcolor{red!65} \\ \hline
