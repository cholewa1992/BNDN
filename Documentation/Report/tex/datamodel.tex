...
Looking at the requirements for \textit{The System} two major entities ere identified, namely the \textit{User} and \textit{Media item} (modelled as entity). Additionally it bacame clear that an entity representing a \textit{Client} was needed in order for the \textit{ShareIt Back End} to be able to differentiate which \textit{Media Item}s belonged to which \textit{Client}. \\
